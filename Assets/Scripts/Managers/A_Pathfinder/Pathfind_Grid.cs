...
public class Pathfind_Grid : MonoBehaviour
{
    ...
    private void Awake()
    {
        ...
        Dictionary<string, A_Pathfind.SurfaceType> tagToSurfaceDictionary = new Dictionary<string, A_Pathfind.SurfaceType>();
        for(int i = customTagStart; i < UnityEditorInternal.InternalEditorUtility.tags.Length; i++)
        {
            ...
            Debug.Log($"Surface {(A_Pathfind.SurfaceType)(i + surfaceIndexOffset) - customTagStart} added to dictionary with key {UnityEditorInternal.InternalEditorUtility.tags[i]}");
        }

        CreateGrid(tagToSurfaceDictionary);
    }

    /// <summary>
    /// Generates the grid by populating it with nodes, starting at the bottom left.
    /// </summary>
    private void CreateGrid(Dictionary<string, A_Pathfind.SurfaceType> tagConversion)
    {
            ...
            for (int y = 0; y < gridSizeY; y++)
            {
                ...
                if (isWalkable)
                {
                    ...
                    if(Physics.Raycast(ray, out hit))
                    {
                        ...
                        tagConversion.TryGetValue(hit.transform.tag, out surfaceType);
                    }
                }
    ...
    private void OnDrawGizmos()
    {
        ...
        if (grid != null && DrawGizmos)
        {
            foreach (Node n in grid)
            {
                ...
                if(n.surfaceType != A_Pathfind.SurfaceType.None && n.surfaceType == A_Pathfind.SurfaceType.Bush)
                {
